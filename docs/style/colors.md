...
| Charcoal hierarchy | `#4A4350` / `#5A5158` / `#6B6270` | `--color-brand-charcoal-2` … `-4` (CSS var) | Charcoal at progressively lighter steps for paragraphs, captions, and meta text. Reach for these via `text-[var(--color-brand-charcoal-2)]` — they're hierarchy variants, not stand-alone brand colors, and intentionally don't get short utility names. |
